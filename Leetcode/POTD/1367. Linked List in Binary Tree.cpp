...
public:
    bool solve(ListNode* head, TreeNode* root){
        if (head == nullptr) return true; // Successfully matched the entire linked list
        if (root == nullptr) return false; // Reached a leaf in the tree, but linked list is not fully matched
